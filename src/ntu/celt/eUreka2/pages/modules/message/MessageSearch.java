...
public class MessageSearch extends AbstractPageMessage {
	@SessionState
	...
	void onActivate(EventContext ec) {
		...
		if(ec.getCount()==2){
			projId = ec.get(String.class, 0);
			searchText = ec.get(String.class, 1);
		}
	}
	Object[] onPassivate() {
		if(searchText==null) 
			return new Object[] {projId};
		else return new Object[] {projId, searchText};
	}
	...
	void setupRender(){
		...
		msgs = msgDAO.searchMessages(getCurUser(), curProj, searchText);
	...
	public BeanModel<Message> getModel(){
		BeanModel<Message> model = beanModelSource.createEditModel(Message.class, _messages);
        model.include("subject","content","flag", "hasRead", "type");
		...
		model.reorder("chkbox","flag","hasRead","subject","content","sender","receiver","date","type");
        return model;
	}
	...
	public Object[] getProjIdAndMsgId(){
		return new Object[]{curProj.getId(), msg.getId()};
	}
