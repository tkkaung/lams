...
public class CheckStatus extends AbstractPageProfiling{
	@Property
	...
	void onActivate(long id) {
		...
		prof = eDAO.getProfilingById(eid);
		if(prof==null)
			throw new RecordNotFoundException(messages.format("entity-not-exists", "Profiling ID", eid));
		project = prof.getProject();
		assessors = getNotSubmitedAssesseesByProfiling(prof); 
//		assessors = getAllAssessees(project); 
	}
	long onPassivate() {
		return eid;
	}
