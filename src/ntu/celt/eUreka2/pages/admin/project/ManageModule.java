...
public class ManageModule extends AbstractPageAdminProject {
	@Property
	...
	void onSuccessFromAssignForm(){
		if(SubmitType.ASSIGN.equals(submitType)){
			...
			for(String mId : selModId){
				...
				if(!curProj.getModules().contains(m) && !curProj.getType().getNonModules().contains(m)){
			...
			for(String mId : selModId){
				...
				if(!curProj.getType().getModules().contains(m)){
