...
*/
	...
	public Object onSuccessFromForm() throws IOException {
		...
		try {
			...
			group.setProject(proj);
			group.setGroupType(groupTypeName);
			...
			nextLine = csvReader.readNext(); // ignore title row (Username, Grp)

			while ((nextLine = csvReader.readNext()) != null) {
				String username = nextLine[0].trim();
				int groupNum = Integer.parseInt(nextLine[1]);
				String groupNumName = null;
				if (nextLine.length > 2) { 
					groupNumName = nextLine[2].trim();
				}
				
				User u = userDAO.getUserByUsername(username);
				if (u != null){
					for (GroupUser gu : group.getGroupUsers()){
						if(gu.getGroupNum() == groupNum ){
							gu.setGroupNumName(groupNumName);
							gu.addUser(u);
					...
					appState.recordWarningMsg("Not found username:" + username);
					countNotFound++;
				}
			}
